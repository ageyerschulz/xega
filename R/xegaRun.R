...
#'
#' Island models are a set of loosely coupled asynchronous processes which communicate either 
#' by file I/O in a shared file system or by message passing (MPI):
#' \itemize{
#' \item For rds, set 
#'       \code{Send="rds", Receive="rds", collectResult="rds"} 
#'       or      \code{Send="rds", Receive="rdsb", collectResult="rds"} for barrier synchronization. 
#' \item For MPI, set 
#'       \code{Send="mpi", Receive="mpi", collectResult="mpi"} 
#'       or      \code{Send="mpi", Receive="mpib", collectResult="mpi"} for barrier synchronization. 
#' }
#'
#' In addition, \code{migrate} specifies the migration policy: 
#'    \itemize{    
#'       \item "No": No genes migrate between islands. Islands run in parallel 
#'             without exchange of genes.
#'       \item "Yes": Islands exchange a set of genes in regular intervals. 
#'             The interval is specified by \code{migrateEvery}.
#'      \item "OnImprovement": Genes emigrate only if the fitness of the island has improved.  
#'    }
#'
#' The selection of emigrants and of the genes which are replaced by immigrants is configured 
#' by \code{selMigrant} and \code{selReplace}. All selection methods may be used.
#' The number of emigrants is defined by \code{Nmigrants}.
#'
#' The communication topology is specified by a communication graph between processes. 
#' At the moment the following communcation topologies (\code{CommunicationTopolgy}) are predefined:
#' ring, bi-directional ring, 2D-torus, 3D-torus, a generalized Petersen graph, and a random graph.
#'
#' \code{AdaptLimit="Slowest"} increases the number of generations of the faster island processes 
#' in such a way to keep all cores busy and that all processes terminate approximately at the same time.
#' \code{MigrationStrategy="Optimist"} does not use a termination protocol with the risk that a bad setting 
#' of the generation limit of a fast process leads to idle cores.
#' \code{MigrationStrategy="Pessimist"} means that the slowest process sends a termination message
#' to the other island processes as soon as it reaches its generation limit. 
#'
#' For convenience, \code{collect=TRUE} means that process \code{0} collects the results of all other 
#' processes and builds a named list which may be written to a file with prefix \code{xegaIResult<Pattern>.rds}  
#' and which is returned. 
#' 
#' We distinguish between 
#' \enumerate{
#' \item \strong{homogeneous island models} (with the same parameters for each island) and
#' \item \strong{heterogeneous island models} (with different parameters for each island).
#'       A set of configurations consists of a list of configurations. A single configuration 
#'       is generated by calling \code{xegaRun()} with \code{Configuration=TRUE}.   
#' }
#'
#'    
#'
...
#'                        Default: \code{NULL}.
#' 
#' @param comment         A text. Default: \code{NULL}.
#'                        Comments may document e.g. hardware configurations for xega 
#'                        in external scripts. 
...
#'
##############
#  Parameters for island models.
##############
#' @param migrate     Determines the type of migration.  
#'                     Default: \code{"No"}. 
#'                     \itemize{
#'                     \item "No":  No migration.
#'                     \item "Yes": Migration in regular intervals (specified by \code{migrateEvery}).
#'                     \item "OnImprovement": Migration of the best gene(s) only if the fitness has improved.
#'                      }
#'
#' @param migrateEvery  Integer. Default: \code{1}. 
#'                      The default behavior is to migrate genes at the end of each generation.
#'                      Gene migration is performed, 
#'                      if \code{0==mod(generation, migrateEvery)}.
#'
#' @param Nmigrants     Number of migrants. Default: \code{1}.
#'
#' @param nrecv         Number of receivers. Default: \code{1}.
#'
...
#'
#' @param torusX        Number of processors in X-axes of torus. Default: \code{5}.
#' @param torusY        Number of processors in Y-axes of torus. Default: \code{2}.
#' @param torusZ        Number of processors in Z-axes of torus. Default: \code{1}.
#' @param pid           Process identifier. (pid in \code{0:(npid-1)}).
#'                      Default: \code{0}.
#' 
#' @param npid          Number of processes. Default: \code{10}.
#'
#' @param SelMigrant    Selection method for emigrant. Default: \code{"TopK"}.
#'
#' @param SelReplace    Selection method for genes replaced by 
#'                      immigrants. Default: \code{"TopK"}.
#'
#' @param CommunicationTopology  Communication topology. Default: \code{"ring"}.
#'                      \itemize{
#'                      \item "ring": Ring of processors. 1 copy of messages is sent from \code{i} to \code{i+1}.
#'                      \item "ring2": Ring of processors. 
#'                        2 copies of messages are sent from \code{i} to \code{i+1} and to \code{i-1}. 
#'                      \item "torus2D": 2D torus of processors. 
#'                            Dimensions set by \code{torusX} and \code{torusY}.
#'                            4 copies of messages are sent to the neighbors of the island. 
#'                      \item "torus3D": 3D torus of processors. 
#'                            Dimensions set by \code{torusX}, \code{torusY}, and \code{torusZ}. 
#'                            6 copies of messages are sent to the neighbors of the island.
#'                      \item "random": Select one or more receivers randomly. 
#'                            Number of receivers set by \code{nrecv} (Default: \code{1}). 
#'                            One  copy of messages is sent to each of the \code{nrecv} random neighbors of 
#'                            the island.
#'                      \item "gPetersen": A generalized Petersen graph. 
#'                            Variant set by \code{GPn} and \code{GPk}.
#'                            The default is the classic Petersen graph 
#'                            GP(n=5, k=2) for \code{10} processes.
#'                            3 copies of messages is sent to the neighbors of the island.
#'                      }
#'
#' 
#' @param AdaptLimit    Adapt the number of generations (Default: \code{"Slowest"}).
#'                      \itemize{
...
#'                      if MPI should be used for parallelization.
#' 
#' @param Send          Message send method. Default: \code{"rds"}.
#'
#' Avalailable methods: 
#' \enumerate{
#'  \item "rds": Message sending via rds-file I/O.
#'  \item "mpi": Message senging via MPI. 
#'  }
#'
#' @param Receive       Message receive method. Default: \code{"rds"}.
#'
#'                      Avalailable methods: 
#'                      \enumerate{
#'                      \item "rds": Message receiving via rds-file I/O. Non-blocking.
#'                      \item "rdsb": Message receiving via rds-file I/O. Blocking (Barrier synchronization).
#'                      \item "mpi": Message receiving via MPI. Non-blocking.
#'                      \item "mpib": Message receiving via MPI. Blocking (Barrier synchronization).
#'  }
#'
#' @param collectResult Result collection method. Default: \code{"rds"}.
#'                      Avalailable methods: 
#'                      \enumerate{
...
#'                      without executing it. This option helps to build lists 
#'                      of different configurations for heterogeneous island models.
#'
